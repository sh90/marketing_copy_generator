...
def send_email_gmail(sender_email: str, app_password: str, receiver_email: str, subject: str, body: str) -> tuple[bool, str]:
    """
    Sends an email via Gmail SMTP (SSL 465). Returns (ok, message).
    Use a Gmail 'App Password' (not your normal password) with 2FA enabled.
    """
...
st.caption("Generate ad copy, social captions, or product descriptions in a selected tone, then run a quick A/B test—and email the results.")
...
def load_sample_row():
    ...
    df = pd.read_csv("sample_product.csv", sep=",", quotechar='"', skipinitialspace=True)
    return df.iloc[random.randint(0, len(df) - 1)].to_dict()
...
# Normalize session state to strings (prevents Streamlit TypeError)
...
if variants:
    ...
    if st.button("Run A/B Simulation"):
        ...
        # Store for email
        st.session_state["last_ab"] = {
            "pickA": pickA, "pickB": pickB,
            "score_A": round(sA, 3), "score_B": round(sB, 3),
            "p_A": round(pA, 3), "p_B": round(pB, 3),
            "rate_A": round(res["rate_A"], 3), "rate_B": round(res["rate_B"], 3),
            "lift_%": round(res["lift"] * 100, 2), "z_score": round(res["z"], 2),
        }

        st.write(st.session_state["last_ab"])
        fig = plot_lift(res)
        st.pyplot(fig)
    ...
    if st.button("Send Email"):
        sender_email, app_password = get_email_creds()
        ...
        if not to:
            ...
            subject = f"[Copy Generator] Results — {product_name or 'Untitled Product'} ({ts})"

            # Build a readable body
            lines = []
            lines.append(f"Product: {product_name} | Brand: {brand} | Price: {price}")
            lines.append(f"Audience: {audience}")
            lines.append(f"Kind: {kind} | Style: {style} | Length: {length}")
            if guidelines:
            ...
            for i, v in enumerate(variants, 1):
                lines.append(f"\nV{i}:\n{v}")
            if "last_ab" in st.session_state:
                ab = st.session_state["last_ab"]
            ...
            ok, msg = send_email_gmail(sender_email, app_password, to, subject, body)
            if ok:
                st.success(msg)
            else:
                st.error(msg)
